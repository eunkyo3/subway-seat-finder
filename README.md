...
./.venv/Scripts/python.exe -m pytest                   # 테스트 (354개)
```
...
시발 감지의 로그 조회 창도 이 시각을 기준으로 잡습니다.

`predict` 가 `predictionAvailable: false` 를 주는 경우는 두 가지입니다 —
통계가 없는 노선(9호선·광역철도), 그리고 **혼잡도 통계가 아직 적재되지 않은 상태**(ETL 미실행).
둘 다 기준 0% 를 '여유'로 포장하지 않고 도착 정보만 반환합니다.
...
화면의 `02 · ARRIVING TRAIN` 패널에서 방면을 직접 고를 수 있고, `자동` 으로 두면 위 추론에 맡깁니다.
추론으로 정해졌을 때는 역명 옆 배지에 `상선 · 자동` 처럼 표시되어, 지금 보고 있는 방면이 내가 고른 것인지 서버가 정한 것인지 구분됩니다.
`direction` 은 `상행`/`내선` 같은 실시간 원본 어휘로 줘도 됩니다 — 서버가 `normalize_direction` 으로 통계 어휘(`상선`/`하선`)에 맞춥니다.

`etaSec` 이 `null` 이면 **도착까지 남은 시간을 모른다**는 뜻이고 `0` 과 다릅니다. 이때 `stationsAway` 가 있으면 몇 정거장 앞인지는 알 수 있습니다 (8호선과 광역철도 일부가 상시 이 상태입니다 — 아래 '알려진 한계' 참고).
...
- **도착 카운트다운이 아예 없는 노선이 있습니다.** 실시간 도착 API 의
  `barvlDt`(카운트다운)가 늘 `0` 으로만 오는 노선이 실측 10개입니다 —
  **서울교통공사 1~9호선 중에서는 8호선**(10,070행 중 유효 0건, 0.0%)이고,
  나머지는 광역철도입니다: 수인분당·경의중앙·경춘·서해·신분당·경강·공항철도 등.
  1~9호선 안에서만 보면 8호선을 뺀 나머지가 3.6~100% 라 8호선만 튀어 보이지만,
  **실제로 영향받는 행은 광역철도 쪽이 2.6배 더 많습니다**(8,894행 vs 22,915행).
  게다가 `arvlCd` 까지 `1`(당역 도착)로 와서, 정작 `arvlMsg2` 는
  `[5]번째 전역` 이라고 말합니다. 코드값을 믿으면 다섯 정거장 밖 열차가
  "0분 후 도착"으로 나갑니다. 그래서 **코드와 메시지가 어긋나면 메시지를 믿고**,
  남은 시간은 미상으로 둡니다. 대신 `[N]번째 전역` 에서 뽑은 **남은 정거장 수**를
  `stationsAway` 로 제공해 "5정거장 전"까지는 정직하게 표시합니다.
  이 노선들은 같은 이유로 **배차간격 보정도 걸리지 않습니다**(간격을 잴 시각이 없습니다).
- **기준 배차간격은 피크 시간대만 캘리브레이션됐습니다.** 수집 서버 로그
  18,421 표본(평일, 모호한 ETA 0·반복 재관측 제거 후)에서 노선×시간대 중앙값을 뽑아,
  표본 30개 이상인 **33개 셀을 `NOMINAL_HEADWAY_MIN_BY_LINE` 에 반영**했습니다.
  커버 범위는 **평일 7·8·9시와 20시뿐**이고, 낮 시간대와 주말은 여전히 노선 구분 없는
  정성값(`NOMINAL_HEADWAY_MIN`)으로 폴백합니다.
  반영 전 상태는 노선별 편차가 **최대 +78.6%** 였습니다. 노선 구분 없는 표가 사실상
  2호선 시각표였기 때문입니다 — 2호선은 오차 1~13%로 맞았지만 배차가 성긴
  5·6·7·9호선은 60~79% 벗어났고, `ratio = 실측/기준` 이 상시 1을 넘어
  **아침 피크 혼잡도를 약 40~50% 과대평가**하고 있었습니다.
  `calibrate_headway` 를 다시 돌리면 반영된 셀은 편차 ±0.2% 로 수렴하므로,
  **표에 편차가 남은 셀이 곧 미반영 목록**입니다.
...
남은 검증 항목:
- **비피크·주말 시간대의 기준 배차간격.** 피크(7·8·9시)와 20시는 실측으로 채웠지만,
  낮 시간대(10~17시)·심야·토요일·일요일은 표본이 없어 정성값 폴백입니다.
  크론(평일 07:00 / 18:00)만으로는 영영 안 채워지므로, 채우려면 수집 창을 넓혀야 합니다.
...
**`barvlDt`(도착까지 남은 초)의 0 은 두 가지 뜻입니다** — 진짜 '도착 직전'과
'카운트다운 정보 없음'. 하루치 로그의 **59%가 0** 이었습니다(실측). `arvlCd` 가
진입(`0`)/도착(`1`)일 때만 0초로 믿고, 나머지는 미상(`None`)으로 둡니다.
이걸 한 값으로 뭉개면 후보 정렬(0 은 falsy)·배차간격·캘리브레이션이 전부 오염됩니다.

**`barvlDt` 는 `recptnDt` 시점의 카운트다운입니다.** 운행이 끝나 원천이 갱신을
멈추면 오래된 "3분 후 도착"이 그대로 남습니다 — 열차가 안 오는데 도착 예정이
뜨는 유령 도착의 원인. live 서빙은 데이터 나이만큼 ETA 를 당기고, 이미 지나간
열차(유예 30초 초과)와 죽은 원천(10분 이상 침묵)은 걸러냅니다. 재생(replay)은
과거 시점을 그대로 보여주는 게 목적이라 이 필터를 적용하지 않습니다.
